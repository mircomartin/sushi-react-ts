        ...
        <div className='order-2 md:order-1'>
          <AnimationOnScroll animateIn='animate__fadeInLeft' animateOnce={true} animatePreScroll={false}>
            <div className='flex flex-col gap-8'>
              <h3 className='text-[#B6262F] text-3xl md:text-4xl font-bold text-center md:text-left uppercase'>{mediaText?.title}</h3>
              {
                mediaText?.description !== undefined
                &&
                <div className='text-lg flex flex-col gap-5 leading-8'>{ReactHtmlParser(mediaText?.description)}</div>
              }
            </div>
          </AnimationOnScroll>
        </div>
        <figure className='order-1 md:order-2 overflow-hidden'>
